...
def initialize_database():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS external_hospitals (
        id TEXT PRIMARY KEY,
        name TEXT,
        address TEXT,
        google_url TEXT
...
def search_hospital():
    ...
    if st.button("Search on Google"):
        ...
        # Add option to manually enter hospital from Google search results
        st.subheader("Book with a Hospital from Google Search")
        
        with st.form("external_hospital_form"):
            hospital_name = st.text_input("Hospital Name (from Google search)")
            hospital_address = st.text_input("Hospital Address")
            hospital_google_url = st.text_input("Google Maps URL (optional)")
            
            submit_button = st.form_submit_button("Add This Hospital")
            ...
            if submit_button:
                if hospital_name and hospital_address:
                    ...
                    # Check if this hospital already exists
                    cursor.execute("SELECT id FROM external_hospitals WHERE name = ? AND address = ?", 
                    ...
                    if existing:
                        ...
                        hospital_id = f"EXT_HOS_{uuid.uuid4().hex[:6]}"
                        cursor.execute("INSERT INTO external_hospitals VALUES (?, ?, ?, ?)",
                                    (hospital_id, hospital_name, hospital_address, hospital_google_url))
                        conn.commit()
                    
                    conn.close()
                    
                    # Store in session state for booking
                    st.session_state.selected_external_hospital = {
                        'id': hospital_id,
                        'name': hospital_name,
                        'address': hospital_address
                    }
                    
                    st.success(f"Added {hospital_name} for appointment booking!")
                    
                    # Link to booking
                    st.markdown("[Click here to book appointment with this hospital](#book-appointment)")
                else:
                    st.warning("Please enter both hospital name and address.")
...
def view_health_history():
  ...
  def patient_dashboard():
    ...
    with tabs[0]:  # Book Appointments
        ...
        st.markdown('<a name="book-appointment"></a>', unsafe_allow_html=True)
        ...
        if patient_data:
            ...
            # Initialize hospital selection state
            if "hospital_selected" not in st.session_state:
                st.session_state.hospital_selected = False
            
            # Option to select hospital type
            hospital_source = st.radio("Select Hospital From:", ["Our System", "Google Search Results"])
            
            if hospital_source == "Our System":
                # Get hospitals from our system
                cursor.execute("SELECT id, name FROM hospitals")
                system_hospitals = cursor.fetchall()
                
                if system_hospitals:
                    system_hospital_dict = {name: id_ for id_, name in system_hospitals}
                    
                    with st.form("book_system_appointment"):
                        hospital_choice = st.selectbox("Choose Hospital", list(system_hospital_dict.keys()))
                            ...
                            if reason:
                                ...
                                try:
                                    cursor.execute("INSERT INTO appointments VALUES (?, ?, ?, ?, ?, ?, ?)",
                                                  (appt_id, patient_id, system_hospital_dict[hospital_choice], 
                                                   "internal", appt_datetime, 
                                                   f"{reason} (Doctor: {doctor_preference})", "Scheduled"))
                                    conn.commit()
                    ...
                    st.info("No hospitals found in our system. Try booking with an external hospital.")
            
            elif hospital_source == "Google Search Results":
                # Option to add hospital from Google
                st.write("Please add a hospital from your Google search:")
                
                # Check if we already have a selected external hospital
                if "selected_external_hospital" in st.session_state:
                    ext_hospital = st.session_state.selected_external_hospital
                    st.info(f"Currently selected: {ext_hospital['name']} at {ext_hospital['address']}")
                    use_current = st.checkbox("Use this hospital", value=True)
                    
                    if not use_current:
                        st.session_state.pop("selected_external_hospital", None)
                        st.rerun()
                
                # If no external hospital is selected, show the form to add one
                if "selected_external_hospital" not in st.session_state:
                    with st.form("add_external_hospital"):
                        hospital_name = st.text_input("Hospital Name")
                        hospital_address = st.text_input("Hospital Address")
                        hospital_google_url = st.text_input("Google Maps URL (optional)")
                        
                        add_button = st.form_submit_button("Add Hospital")
                ...
                # Book appointment with the external hospital
                if "selected_external_hospital" in st.session_state:
                    ext_hospital = st.session_state.selected_external_hospital
                    
                    with st.form("book_external_appointment"):
                        st.write(f"Booking with: **{ext_hospital['name']}**")
                        appointment_date = st.date_input("Appointment Date", min_value=datetime.now().date())
                        appointment_time = st.time_input("Appointment Time")
                        reason = st.text_area("Reason for Visit")
                        doctor_preference = st.text_input("Preferred Doctor (optional)")
                        notes = st.text_area("Additional Notes (e.g., instructions to contact hospital manually)")
                        ...
                        if submit_button:
                            if reason:
                                ...
                                try:
                                    cursor.execute("INSERT INTO appointments VALUES (?, ?, ?, ?, ?, ?, ?)",
                                                  (appt_id, patient_id, ext_hospital['id'], 
                                                   "external", appt_datetime, 
                                                   f"{reason} (Doctor: {doctor_preference})\n{notes}", "Pending"))
                                    conn.commit()
                                    st.success(f"""
                                        Appointment request saved successfully!
                                        
                                        NOTE: Since this is an external hospital (found via Google), 
                                        you may need to contact them directly to confirm your appointment.
                                        This system only tracks your appointment request.
                                    """)
                                except Exception as e:
                                    st.error(f"Error booking appointment: {e}")
                            else:
        ...
        if patient_data:
            ...
            cursor.execute("""
                SELECT 
                    a.id, 
                    CASE 
                        WHEN a.hospital_type = 'internal' THEN h_int.name 
                        ELSE h_ext.name 
                    ...
                    a.status 
                FROM appointments a 
                LEFT JOIN hospitals h_int ON (a.hospital_id = h_int.id AND a.hospital_type = 'internal')
                LEFT JOIN external_hospitals h_ext ON (a.hospital_id = h_ext.id AND a.hospital_type = 'external')
                WHERE a.patient_id = ? 
            ...
            if appointments:
                df = pd.DataFrame(appointments, columns=["Appointment ID", "Hospital", "Hospital Type", "Date & Time", "Reason", "Status"])
                st.dataframe(df)
